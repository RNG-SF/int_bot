...
].filter(p => p && p.enabled !== false && /^\d{8}$/.test(String(p.birthday || "")));
...
const targetName = person?.nickname || person?.fullName || "kamu";
const messageKey = person?.hbdMessageNo || person?.id || "";
let messageTemplate =
    window.HBD_MESSAGE_DATA?.[messageKey] ||
    window.HBD_MESSAGE_DATA?.default ||
    "Selamat ulang tahun, {name}. 🤍";
const message = String(messageTemplate).replaceAll("{name}",targetName);
